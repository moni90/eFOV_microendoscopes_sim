function [A_or,C_or] = segment_CaImAn(caiman_path, Y, SNR_thr, merging_thr, min_n_px)
%Y = 3d time series

addpath(genpath(fullfile(caiman_path,'utilities')));
addpath(genpath(fullfile(caiman_path,'deconvolution')));
curr_dir = pwd;
cd(caiman_path);
%% load file
gcp;                            % start cluster
...
options = CNMFSetParms(...   
    ...
    'merge_thr', merging_thr,...                        % merging threshold  
    'nb',1,...                                  % number of background components    
    'min_SNR',SNR_thr,...                             % minimum SNR threshold
    'space_thresh',0.5,...                      % space correlation threshold
    'cnn_thr',0.2,...                           % threshold for CNN classifier    
    'min_size_thr',min_n_px...                  % min number of pixels
    );
%% Data pre-processing
...
[A_or,C_or,S_or,P_or,~,srt] = order_ROIs(A2,C2,S2,P2); % order components
K_m = size(C_or,1);
